...
import { CheckCircle2, CalendarIcon, Clock, Paperclip, Target, ArrowUpDown, HelpCircle, EyeClosed, AlertCircle, Sparkles, Loader2 } from 'lucide-react'
      ...
      {!summaryLoading && (
        uncategorizedCount > 0 ? (
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-5">
            <button
              type="button"
              className="flex-1 min-w-0 flex items-center justify-between gap-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-lg px-4 py-2.5 cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors"
              onClick={() => setDrillDown({
            ...
            {/* The AI pass sits beside the banner, not inside it: the banner is
                itself a button and a nested one would neither be valid markup
                nor reachable without also firing the drill-down. */}
            {agentsEnabled && (
              <button
                type="button"
                disabled={autoCategorizeMutation.isPending}
                onClick={() => autoCategorizeMutation.mutate()}
                className="shrink-0 inline-flex items-center justify-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:cursor-not-allowed disabled:opacity-60"
              >
